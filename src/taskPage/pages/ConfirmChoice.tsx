...
const ConfirmChoice = () => {
  ...
  const [language, setLanguage] = useState<SingleValue<Language>>({
    label: "",
    value: "",
  });
  ...
  useEffect(() => {
    ...
    getTaskById();
  }, [taskId, token, sendRequest]);

  const handleLanguageChange = (lang: SingleValue<Language>) => {
    setLanguage(lang);
                  ...
                  <span className="task-summary__category">
                    {
                      categoryTranslation.find(
                        (category) => category.categoryId === task.category
                      )?.translation
                    }
                  </span>
                </p>
