...
import { AuthFieldsPattern, AuthValues, FieldProps } from "../../types/auth";
import RegisterSchema from "../../shared/utils/formSchemas/registerSchema";
...
import { useScrollBlock } from "../../shared/hooks/blockScrollHook";
import { AlertColor } from "@mui/material";

const loginFields: AuthFieldsPattern[] = [
  {
    name: "login",
    label: "Login:",
    type: "text",
...
const registerFields: AuthFieldsPattern[] = [
  ...loginFields,
  {
...
const AuthForm = () => {
  ...
  const handleAuthSubmit = async (formValues: AuthValues) => {
  ...
  const handleCheckUsername = async (formFields: FormikValues) => {
    if (
  ...
  return (
    <>
      ...
      <Formik
        initialValues={isRegistration ? initFormRegistryState : initLoginState}
      ...
      >
        {(props: FormikProps<AuthValues>) => {
          const everyItemHasValue = Object.values(props.values).every(hasValue);
          return (
            <Form onChange={(formFields) => handleCheckUsername(formFields)}>
              ...
              <div className="center">
                <span className="question">
                  {isRegistration ? "Masz już konto? " : "Nie masz konta? "}
                </span>
                {isRegistration ? (
                  <NavLink to="/login" exact className="question-navlink">
                    Zaloguj się
                  </NavLink>
                ) : (
                  <NavLink to="/register" exact className="question-navlink">
                    Zarejestruj się
                  </NavLink>
                )}
              </div>
